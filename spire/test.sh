...
apply_server_config() {
	...
	kubectl apply -f ./spire-namespace.yaml > /dev/null
	kubectl apply -f ./server-account.yaml > /dev/null
	kubectl apply -f ./server-cluster-role.yaml > /dev/null
	kubectl apply -f ./server-configmap.yaml > /dev/null
	kubectl apply -f ./spire-bundle-configmap.yaml > /dev/null
	kubectl apply -f ./server-statefulset.yaml > /dev/null
	kubectl apply -f ./server-service.yaml > /dev/null
	echo "${green}ok.${norm}"
}
